...
_data = r"./"
_file = (
    r"F:\_PROJECT_\CON_2023_VSS_AutoLevelingValve\DATAFromIWON\Test_2nd\MOVIE\000.mp4"
)
...
def quad_area(data):
    ...
    e3 = c4 - c3
    e4 = c1 - c4
    a = -0.5 * (np.cross(-e1, e2, axis=1) + np.cross(-e3, e4, axis=1))
    return a

    corners2 = np.array([c[0] for c in corners])
    data = pd.DataFrame(
        {"x": corners2[:, :, 0].flatten(), "y": corners2[:, :, 1].flatten()},
